...
from const_config import CUSTOM_CNN_V2
...
class CustomCNNv2(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(YOLO_LABELS_PER_IMAGE):
            j = 4 * i
            results[i] = self.image_part(x[:, :, :, j:j+4])
        return torch.cat(results, 1).reshape(-1, YOLO_OUTPUTS_PER_LABEL_ONLY_CLASSES)
    
    def load(self):
        with open(f"{MODEL_PATH}{CUSTOM_CNN_V2}", "rb") as file:
            self.load_state_dict(torch.load(file))
    
    def save(self):
        with open(f"{MODEL_PATH}{CUSTOM_CNN_V2}", "wb") as file:
